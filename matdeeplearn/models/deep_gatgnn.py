...
class GATGNN_AGAT_LAYER(MessagePassing):
    def __init__(
        ...
        **kwargs
        ...
        super(GATGNN_AGAT_LAYER, self).__init__(
            aggr="add", flow="target_to_source", **kwargs
        )
    ...
    def message(self, edge_index_i, x_i, x_j, size_i, edge_attr):
        ...
        alpha = getattr(F, self.act)(
            (torch.cat([out_i, out_j], dim=-1) * self.att).sum(dim=-1)
        )
...
class DEEP_GATGNN(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.global_att_LAYER = GATGNN_GIM1_globalATTENTION(
            dim1, act, batch_norm, batch_track_stats, dropout_rate
        )
        ##================================
        ...
        for i in range(gc_count):
            ...
            if self.batch_norm == "True":
                ...
                bn = DiffGroupNorm(
                    gc_dim, 10, track_running_stats=self.batch_track_stats
                )
                self.bn_list.append(bn)
    ...
    def forward(self, data):
        ...
        if self.pool_order == "early":
            ...
            latentEmbG = out_x.clone()  # Extract embeddings after pooling
        ...
        if out.shape[1] == 1:
            return out.view(-1), latentEmbG
        else:
            return out, latentEmbG
