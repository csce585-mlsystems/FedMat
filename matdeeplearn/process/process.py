...
def split_data_ood(
    ...
    fold_name = "OFM_" + dataset_name + "_" + tg + "_target_clusters50"
    train_fold_path = (
        "folds/" + dataset_name + "_folds/train/" + fold_name + "_train.json"
    )
    val_fold_path = "folds/" + dataset_name + "_folds/val/" + fold_name + "_val.json"
    test_fold_path = "folds/" + dataset_name + "_folds/test/" + fold_name + "_test.json"

    with open(train_fold_path, "r") as train_fold_reader:
        train_folds = json.load(train_fold_reader)
    with open(val_fold_path, "r") as val_fold_reader:
        val_folds = json.load(val_fold_reader)
    with open(test_fold_path, "r") as test_fold_reader:
        test_folds = json.load(test_fold_reader)

    id_train = train_folds[str(fold)]
    id_val = val_folds[str(fold)]
    id_test = test_folds[str(fold)]

    # id_train = [int(idTr) for idTr in id_train]
    # id_val = [int(idVa) for idVa in id_val]
    # id_test = [int(idTe) for idTe in id_test]
    ...
    #     generator=torch.Generator().manual_seed(seed),
    # )
    train_dataset = torch.utils.data.Subset(dataset, id_train)
    val_dataset = torch.utils.data.Subset(dataset, id_val)
    test_dataset = torch.utils.data.Subset(dataset, id_test)
    print(
        "train length:",
        len(id_train),
        "val length:",
...
def get_dataset(data_path, target_index, reprocess="False", processing_args=None):
    ...
    if os.path.exists(os.path.join(data_path, processed_path, "data.pt")) == True:
        ...
        if os.path.exists(os.path.join(data_path, processed_path, "data.pt")) == True:
            ...
            os.path.exists(os.path.join(data_path, processed_path, "data0.pt")) == True
        ):
            dataset = StructureDataset_large(
                data_path,
                processed_path,
...
def process_data(data_path, processed_path, processing_args):
    ...
    for index in range(0, len(target_data)):
        ...
        if processing_args["data_format"] != "db":
            ...
            if index == 0 and processing_args["data_format"] == "cif":
                struc = Structure.from_file(
                    os.path.join(
                        data_path, structure_id + "." + processing_args["data_format"]
                    )
                )
                ...
                if result > 100:
                    print(
                        "materials structures not stable, please provide a stable materials dataset"
                    )
                    exit()
        else:
